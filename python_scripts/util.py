...
def cartesian_basis_vector(z_axis):
  ...
  assert z_axis.size is 3, "a3 should have length of 3"
...
def direction_cosine(axis):
  ...
  """
  assert axis.size is 3, "axis should have length of 3 but is: {}".format(
      axis.size)
...
def HCP_coordinates(diameter=5, lattice_constant=1):
  """
  return coordinates of a hexagonal close-packed (HCP) lattice in a circular area.
