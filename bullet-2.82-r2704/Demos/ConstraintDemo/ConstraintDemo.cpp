...
//		btCollisionShape* pDoorShape = new btBoxShape(btVector3(5.0f, 5.0f, 0.2f));
//		m_collisionShapes.push_back(pDoorShape);
//		btTransform doorTrans;
//		doorTrans.setIdentity();
//		doorTrans.setOrigin(btVector3(-0.0f, -5.0f, 0.0f));
//		btRigidBody* pDoorBody = localCreateRigidBody(1.0, doorTrans, pDoorShape);
//		pDoorBody->setActivationState(DISABLE_DEACTIVATION);
//		const btVector3 btPivotA(0.f, 0.f, 0.0f); // right next to the door slightly outside
//		btVector3 btAxisA(0.0f, 1.0f, 0.0f); // pointing upwards, aka Y-axis
//
//		spDoorHinge = new btHingeConstraint(*pDoorBody, btPivotA, btAxisA);
//		
...
//#endif
	...
	{ 
		...
		btTransform tr;
		tr.setIdentity();
		tr.setOrigin(btVector3(btScalar(5), btScalar(0.), btScalar(0.)));
		btRigidBody* pBodyA = localCreateRigidBody( 1.0f, tr, shape);
		pBodyA->setActivationState(DISABLE_DEACTIVATION);
		// dynamic bodyB:
		tr.setIdentity();
		tr.setOrigin(btVector3(btScalar(0.), btScalar(0.), btScalar(0.)));
		btRigidBody* pBodyB = localCreateRigidBody(1000.0, tr, shape);
		pBodyB->setActivationState(DISABLE_DEACTIVATION);
		// add some data to build constraint frames
		btVector3 axisA(0.f, 1.f, 0.f); 
		btVector3 axisB(0.f, 1.f, 0.f); 
		btVector3 pivotA(-3.f, 0.f, 0.f);
		btVector3 pivotB( 2.f, 0.f, 0.f);
		...
		// draw con	straint frames and limits for debugging
