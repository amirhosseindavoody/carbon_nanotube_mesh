...
void	DemoApplication::shootBox(const btVector3& destination)
{
	...
	if (m_dynamicsWorld)
	{
		...
		btRigidBody* body = this->localCreateRigidBody(mass, startTransform,m_shootBoxShape, COL_BOX, COL_EVERYTHING);
...
void	DemoApplication::mouseMotionFunc(int x,int y)
{
	...
	{
		btScalar mouseSpeedScale = btScalar(20.);
		if(m_mouseButtons & 2)
		{
			btVector3 hor = getRayTo(0,0)-getRayTo(1,0);
			btVector3 vert = getRayTo(0,0)-getRayTo(0,1);
			btScalar multiplierX = btScalar(0.001)*mouseSpeedScale;
			btScalar multiplierY = btScalar(0.001)*mouseSpeedScale;
			if (m_ortho)
			{
				multiplierX = 1;
				multiplierY = 1;
		...
		{
			m_cameraDistance -= dy * btScalar(0.02f)*mouseSpeedScale;
			if (m_cameraDistance<btScalar(0.1))
				m_cameraDistance = btScalar(0.1);
...
//		m_dynamicsWorld->getDebugDrawer()->drawAabb(aabbMin,aabbMax,btVector3(1,1,1));
		...
		//If this gets commented out then we do not see any of the object drawn in the window. The wireframe still works though
		if (!(getDebugMode()& btIDebugDraw::DBG_DrawWireframe) && body->isDrawable())
		{
			switch(pass)
			{
